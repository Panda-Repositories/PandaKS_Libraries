...
-- Panda V2 (Class A)
local function ValidateKey(UserKey, ServiceID, Client_HWID)
	local Url = "https://pandadevelopment.net/v2_validation?key=" .. UserKey .. "&service=" .. ServiceID .. "&hwid=" .. Client_HWID
	...
	if not Success then
		warn("[Panda] Validation error: " .. tostring(Response))
		return false
	end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	-- HERE IS WHERE THE PELINDA KEY SYSTEM MODIFICATION BEGINS
	if (Settings.KeySystem) then
		...
		-- Handle new Pelinda key system
		if Settings.KeySettings.ServiceID and Settings.KeySettings.Client_HWID then
			if Settings.KeySettings.GrabKeyFromSite then
				-- Copy key URL to clipboard or display it
				local KeyUrl = "https://pandadevelopment.net/getkey?service=" .. Settings.KeySettings.ServiceID .. "&hwid=" .. Settings.KeySettings.Client_HWID
				if setclipboard then
					setclipboard(KeyUrl)
					print("[Panda] Key URL copied to clipboard: " .. KeyUrl)
				else
					print("[Panda] Get your key at: " .. KeyUrl)
				end
			end
		else
			-- Fallback to original key system
		...
		if isfile and isfile(RayfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			if Settings.KeySettings.ServiceID and Settings.KeySettings.Client_HWID then
				-- For Pelinda system, validate the saved key
				local SavedKey = readfile(RayfieldFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
				if ValidateKey(SavedKey, Settings.KeySettings.ServiceID, Settings.KeySettings.Client_HWID) then
					Passthrough = true
				end
			else
				-- Original key system logic
        ...
        if not Passthrough then
			...
			-- Update note message for Pelinda system
			if Settings.KeySettings.ServiceID and Settings.KeySettings.Client_HWID then
				if Settings.KeySettings.GrabKeyFromSite then
					KeyMain.NoteMessage.Text = "Get your key at: https://pandadevelopment.net/getkey?service=" .. Settings.KeySettings.ServiceID .. "&hwid=" .. Settings.KeySettings.Client_HWID
				else
					KeyMain.NoteMessage.Text = Settings.KeySettings.Note or "Enter your Pelinda key"
				end
			end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if Settings.KeySettings.ServiceID and Settings.KeySettings.Client_HWID then
					-- Use Pelinda validation
					if ValidateKey(KeyMain.Input.InputBox.Text, Settings.KeySettings.ServiceID, Settings.KeySettings.Client_HWID) then
						KeyFound = true
