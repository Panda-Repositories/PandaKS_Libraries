...
function Crypt.XorDecrypt(self, Encrypted, Key)
	if CurrentENV ~= PandaAuth["Match\0\0\0\0"] or PandaAuth["Match\0\0\0\0"] ~= getfenv(2) or PandaAuth["Secure_EQ\0\0\0\0"] ~= info(2, "f") then while true do end return end
...
PandaAuth[SetFunc] = function(self, Func) -- SetFunc
	if (self ~= PandaAuth and self ~= PandaAuthTable) or Func ~= self[GetFunc] then while true do end return end
...
PandaAuth[SetENV] = function(self, ENV) -- SetENV
	if (self ~= PandaAuth and self ~= PandaAuthTable) or ENV ~= self[GetENV] then while true do end return end
	
	CurrentENV = ENV
end

PandaAuth[SetInternal] = function(self, Settings) -- SetInternal
	if (self ~= PandaAuth and self ~= PandaAuthTable) or CurrentENV ~= self[GetENV] then while true do end return end
...
PandaAuth[GetInternal] = function(self) -- GetInternal
	if (self ~= PandaAuth and self ~= PandaAuthTable) and CurrentENV ~= self[GetENV] then while true do end return end
...
PandaAuth[SHA256] = function(self, Str, n1, n2, Key) -- SHA256
	if (self ~= PandaAuth and self ~= PandaAuthTable) or CurrentENV ~= self[GetENV] or self[GetENV] ~= getfenv(2) or self[GetFunc] ~= info(2, "f") then while true do end return end
...
PandaAuth[ValidateKey] = function(self, Key) -- ValidateKey (Currently using failsafeValidation)
	...
	--if CompareResponse.Body == response.Body then while true do end return end
...
PandaAuth[ResetHWID] = function(self, Key) -- ResetHWID
	if (self ~= PandaAuth and self ~= PandaAuthTable) or CurrentENV ~= self["Match\0\0\0\0"] then while true do end return end
...
local Metatable = {
	__index = function(self, key)
		...
		if info(4, "s") == "[C]" and info(4, "f") ~= pcall or info(3, "s") == "[C]" and info(3, "f") or info(2, "s") == "[C]" and info(2, "f") or info(1, "s") == "[C]" and info(1, "f") then
